...
public final class HelperXOM {

	public static Document getXML(List<JavaBeans1> javaBeans1s) {
		Document document = null;
		Element list = new Element("list");
		for (int i = 0; i < javaBeans1s.size(); i++) {
			Element object = new Element("object");
			list.appendChild(object);
			Attribute attributeClass = new Attribute("class", javaBeans1s
					.get(i).getClass().toString().substring(6));
			...
			Text textFlag = new Text(
					String.valueOf(javaBeans1s.get(i).isFlag()));
			...
			Attribute attributeClass2 = new Attribute("class", javaBeans1s
					.get(i).getJavaObject().getClass().toString().substring(6));
			object2.addAttribute(attributeClass2);

			Element id2 = new Element("id");
			object2.appendChild(id2);
			Text textId2 = new Text(String.valueOf(javaBeans1s.get(i)
					.getJavaObject().getId()));
			id2.appendChild(textId2);

			Element name2 = new Element("name");
			object2.appendChild(name2);
			Text textName2 = new Text(javaBeans1s.get(i).getJavaObject()
					.getName());
			name2.appendChild(textName2);

			Element mass = new Element("mass");
			object.appendChild(mass);
			Attribute attributeMass = new Attribute("length",
					String.valueOf(javaBeans1s.get(i).getMass().length));
			...
			for (int j = 0; j < javaBeans1s.get(i).getMass().length; j++) {
				...
				Attribute attributeIndex = new Attribute("index",
						String.valueOf(k));
				void1.addAttribute(attributeIndex);

				Text textValue = new Text(String.valueOf(String
						.valueOf(javaBeans1s.get(i).getMass()[j])));
